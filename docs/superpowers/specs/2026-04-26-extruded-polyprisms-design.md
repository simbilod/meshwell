...
Add a `_StructuredPolyPrism` entity to meshwell that produces a **swept,
...
## Public API — unified through `PolyPrism`

Users see **one class**: `PolyPrism`. Adding `n_layers=` switches it to
structured mode. Under the hood, `PolyPrism.__new__` dispatches to a
private subclass `__StructuredPolyPrism` (defined in
`meshwell/structured_polyprism.py`) when `n_layers` is supplied.

```python
PolyPrism(
    polygons,
    buffers,                              # all-zero values when n_layers is given
    n_layers: list[int] | None = None,    # NEW; triggers structured mode
    recombine: bool = False,              # NEW; pass-through to geo.extrude
    physical_name=...,
    mesh_order=...,
    mesh_bool=True,
    ...
    rotation_point=None,
    rotation_angle=0.0,
    additive=False,                       # only for unstructured path
    subdivision=None,                     # only for unstructured path
)
```

**Mode selection:**
- `n_layers is None` → existing unstructured behavior. No semantic change
  for any current `PolyPrism` user.
- `n_layers is not None` → `__new__` returns a `__StructuredPolyPrism`
  instance (subclass of `PolyPrism`). The constructor enforces the
  structured-mode constraints below and the cascade picks it up via
  `isinstance(ent, __StructuredPolyPrism)`.

**Validation at construction (structured mode only):**
...
- z keys strictly increasing.
- `additive` and `subdivision` must be at their defaults (raise if a user
  combines them with `n_layers`; those are unstructured-only knobs).

**Why subclass-via-`__new__` rather than two public classes:**
- One name to remember; one location in autocomplete; existing user code
  is unchanged.
- The two classes do share ~75% of their state and serialization, so a
  subclass relationship is the most accurate expression of "this is a
  PolyPrism with extra meshing behavior."
- `isinstance(ent, PolyPrism)` continues to be true for both, which lets
  any current code that filters by `PolyPrism` keep working.
- Future `n_layers`-derived behaviors (revolve / twist) attach to the
  same dispatch point.

**Serialization** stays as a single type:
- `to_dict` returns `type: "PolyPrism"` for both modes; structured-mode
  instances additionally serialize `n_layers` and `recombine`.
- `from_dict` reads `n_layers` and routes through the same `__new__`
  dispatch — no separate "type" field needed.

## Architecture overview

The lifecycle of a `_StructuredPolyPrism` is split between the CAD stage
...
def resolve_structured_slabs(entities: list[Any]) -> list[Slab]:
    """Decompose _StructuredPolyPrism entities into 3D-disjoint slabs.

    Steps:
    1. For each _StructuredPolyPrism, expand into per-z-interval slabs
...
| File | Change |
|------|--------|
| `meshwell/polyprism.py` | Add `__new__` dispatch + accept `n_layers` / `recombine` kwargs in `__init__` (route to subclass when set); update `to_dict` / `from_dict` to round-trip `n_layers` / `recombine` |
| `meshwell/structured_polyprism.py` | NEW — `__StructuredPolyPrism` (private subclass of `PolyPrism`), `Slab`, `resolve_structured_slabs`, `_StructuredPhantom`, `apply_structured_slabs`, sidecar JSON helpers |
...
| `meshwell/cad_occ.py` | Plumb `structured_slabs_out` into `prepare_entities`; expose captured slabs |
| `meshwell/cad_gmsh.py` | Plumb `structured_slabs_out` into `prepare_entities` |
| `meshwell/occ_xao_writer.py` | Write / read structured-slabs sidecar JSON next to `.xao` |
| `meshwell/orchestrator.py` | Read sidecar; populate `ModelManager.structured_slabs` before mesh stage |
| `meshwell/mesh.py` | Call `apply_structured_slabs` in `process_geometry`; read sidecar in `load_xao_file` |
| `meshwell/utils.py` | (no change needed — single "PolyPrism" deserialize hook covers both modes) |
| `meshwell/__init__.py` | Export remains `PolyPrism` (no new public class) |
| `tests/test_structured_polyprism.py` | NEW — tests above |
| `docs/prisms.py` | Add structured-mode example showing `PolyPrism(..., n_layers=...)` |
...
- **Risk:** Users may expect tapered structured prisms (the `buffers`
  ...
  **Mitigation:** Document explicitly in docstring that `_StructuredPolyPrism`
