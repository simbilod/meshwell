...
def test_unified_pipeline():
    ...
    # Test OCC backend
    m_occ = generate_mesh([surf], dim=2, backend="occ", default_characteristic_length=0.1)
    assert m_occ is not None
    assert len(m_occ.points) > 0
    
    # Test GMSH backend
    m_gmsh = generate_mesh([surf], dim=2, backend="gmsh", default_characteristic_length=0.1)
