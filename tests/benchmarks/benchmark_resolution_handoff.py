...
def test_benchmark_resolution_handoff():
    ...
    resolution_specs = {name: [ConstantInField(resolution=0.1, apply_to="curves", sharing=physical_names)] for name in physical_names}
    ...
    start = time.time()
    m._apply_entity_refinement(boundary_delimiter="None", resolution_specs=resolution_specs)
    end = time.time()
