...
def _coincident_unshared_surfaces() -> list[tuple[int, int]]:
    ...
    return [
        (a, b) for a, b in itertools.combinations(boxes, 2) if boxes[a] == boxes[b]
    ]
...
def test_cohort_bot_face_shared_with_unstructured_substrate(tmp_path, perturbation):
    ...
    left = PolyPrism(box(0, 0, 60, 40), {1.0: 0.0, 2.0: 0.0}, physical_name="left", structured=True)
    right = PolyPrism(box(60, 0, 100, 40), {1.0: 0.0, 2.0: 0.0}, physical_name="right", structured=True)
