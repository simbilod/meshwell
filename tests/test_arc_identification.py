...
import gmsh
from meshwell.geometry_entity import GeometryEntity, DecompositionSegment
...
def test_decompose_vertices_no_arcs():
    ...
    segments = ge.decompose_vertices(vertices, identify_arcs=False, min_arc_points=4, arc_tolerance=1e-3)
...
def test_decompose_vertices_perfect_arc():
    ...
    segments = ge.decompose_vertices(vertices, identify_arcs=True, min_arc_points=4, arc_tolerance=1e-3)
...
def test_decompose_vertices_noisy_arc():
    ...
    segments = ge.decompose_vertices(vertices, identify_arcs=True, min_arc_points=4, arc_tolerance=1e-2)
    
    assert len(segments) == 1
    assert segments[0].is_arc
    assert segments[0].radius > 0.9 and segments[0].radius < 1.1
...
def test_polyprism_arc_instantiate_gmsh():
    ...
    pp = PolyPrism(poly, buffers={0: 0, 1: 0}, identify_arcs=True, min_arc_points=4, arc_tolerance=1e-3)
...
def test_polyprism_arc_error_no_extrude():
    ...
    with pytest.raises(NotImplementedError, match="Arc identification is currently only supported for PolyPrism when extrude=True"):
